...
import { Alert, Button, Card, Col, Empty, Form, Input, Modal, Row, Space, Spin, Tag, Typography } from "antd";
import { useEffect, useState } from "react";

import { PageHeader } from "../../../components/layout/app-shell";
import { createWechatMpAccount, fetchWechatMpAccounts, testWechatMpAccount } from "../../../lib/api";
...
export function WechatMpAccountsPage() {
  ...
  return <WechatMpLayout>
    ...
    <Card extra={<Button icon={<ReloadOutlined />} onClick={() => void load()} loading={loading}>刷新</Button>}>
      {loading ? <Spin /> : accounts.length === 0 ? <Empty description="尚未绑定公众号账号" /> : <Row gutter={[16, 16]}>{accounts.map((account) => <Col xs={24} md={12} lg={8} key={account.id}><Card size="small" title={account.name} extra={<Tag color={account.connection_status === "connected" ? "green" : "default"}>{account.connection_status}</Tag>}><Space direction="vertical"><Text type="secondary">AppID: {account.app_id}</Text><Text type="secondary">AppSecret: 已加密保存，不可查看</Text><Button icon={<ApiOutlined />} loading={testing === account.id} onClick={() => void test(account.id)}>测试连接</Button></Space></Card></Col>)}</Row>}
    </Card>
