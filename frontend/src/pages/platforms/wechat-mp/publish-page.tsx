...
import { fetchWechatMpAccounts, fetchWechatMpArticles, pollWechatMpPublishJob, publishWechatMpArticle, syncWechatMpDraft } from "../../../lib/api";
...
export function WechatMpPublishPage() {
  ...
  return <WechatMpLayout><PageHeader eyebrow="WeChat MP / Publish" title="草稿同步与发布" description="先同步公众号草稿，再明确选择立即发布或定时发布。立即发布必须二次确认。" />
    ...
    <Card title="发布任务状态" extra={job && <Button icon={<ReloadOutlined />} loading={busy} onClick={() => void poll()}>查询状态</Button>} style={{ marginTop: 16 }}>{job ? <Space direction="vertical"><Tag color={job.status === "published" ? "green" : "blue"}>{job.status}</Tag><Text>任务 #{job.id}，草稿同步 #{job.draft_sync_id}</Text>{job.error_message && <Text type="danger">{job.error_message}</Text>}</Space> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="本次会话尚无发布任务" />}</Card>
  </WechatMpLayout>;
}
