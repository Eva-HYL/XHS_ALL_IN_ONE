...
import { createWechatMpArticle, fetchModelConfigs, fetchWechatMpArticle, fetchWechatMpPrompts, generateWechatMpCover, generateWechatMpImage, generateWechatMpPrompts, regenerateWechatMpPrompt, updateWechatMpPrompt } from "../../../lib/api";
import type { ModelConfig, WechatMpArticle, WechatMpAsset, WechatMpImagePrompt } from "../../../types";
...
export function WechatMpWriterPage() {
  ...
  useEffect(() => {
    void fetchModelConfigs("image").then(({ items }) => {
  ...
  async function createArticle() { if (!title.trim() || !topic.trim()) { setError("请填写标题和主题。"); return; } setBusy(true); setError(null); try { const next = await createWechatMpArticle({ title: title.trim(), topic: topic.trim(), source_material: material, target_reader: reader, tone, illustration_skill: skill }); setArticle(next); setParams({ article: String(next.id) }); const nextPrompts = await generateWechatMpPrompts(next.id, skill); setPrompts(nextPrompts); setArticle(await fetchWechatMpArticle(next.id)); setNotice("文章、排版和配图提示词已生成。"); } catch { setError("文章工作流生成失败，请确认文本模型配置。"); } finally { setBusy(false); } }
  ...
  async function generateImage(prompt: WechatMpImagePrompt) { setBusy(true); try { const asset = await generateWechatMpImage(prompt.id, { image_model: imageModel, size: "16:9" }); setAssets((items) => [asset, ...items]); setArticle(await fetchWechatMpArticle(prompt.article_id)); setNotice("公众号配图已生成并回填预览。"); } catch { setError("图片生成失败，请确认图片模型配置。"); } finally { setBusy(false); } }
  async function generateCover() { if (!article) return; setBusy(true); try { const asset = await generateWechatMpCover(article.id, { image_model: imageModel, size: "16:9" }); setAssets((items) => [asset, ...items.filter((item) => item.role !== "cover")]); setArticle(await fetchWechatMpArticle(article.id)); setNotice("封面已生成，现在可以同步公众号草稿。"); } catch { setError("封面生成失败，请确认图片模型配置。" ); } finally { setBusy(false); } }
  ...
  return <WechatMpLayout>
    ...
    {article && <><Row gutter={[16, 16]}><Col xs={24} lg={12}><Card title="2-3. 格式化预览" extra={<Space><Tag>修订 {article.revision}</Tag><Tag>{article.status}</Tag></Space>}><Text strong>{article.title}</Text><Paragraph type="secondary">{article.digest}</Paragraph><Paragraph type="secondary">已记录模型费用：¥{String(article.cost_estimate.total_yuan ?? "0.0000")}</Paragraph><div style={{ borderTop: "1px solid #eee", paddingTop: 12 }} dangerouslySetInnerHTML={{ __html: article.html_body }} /></Card></Col><Col xs={24} lg={12}><Card title="4. 生成配图提示词"><Paragraph>当前技能：<Text code>{skill}</Text>。选择 <Text code>none</Text> 时仅禁用图片生成，不会禁用提示词生成。</Paragraph><Button type="primary" icon={<PictureOutlined />} loading={busy} onClick={() => void makePrompts()}>重新生成提示词</Button></Card></Col></Row>
      <Card title="5. 编辑提示词并生成图片" style={{ marginTop: 16 }}><Space direction="vertical" size={12} style={{ width: "100%" }}><Select placeholder="使用后端默认图片模型" allowClear value={imageModel} onChange={setImageModel} options={imageModels.map((model) => ({ value: model.model_name, label: `${model.name}${model.is_default ? "（默认）" : ""}` }))} /><Button type="primary" icon={<PictureOutlined />} disabled={skill === "none"} loading={busy} onClick={() => void generateCover()}>生成 16:9 公众号封面</Button>{prompts.length === 0 ? <Empty description="提示词生成后在此编辑并生图" /> : prompts.map((prompt) => <Card key={prompt.id} size="small" title={`段落 #${prompt.section_id}`} extra={<Tag>{prompt.status}</Tag>}><TextArea value={prompt.editable_prompt} onChange={(e) => setPrompts((items) => items.map((item) => item.id === prompt.id ? { ...item, editable_prompt: e.target.value } : item))} rows={3} /><Space style={{ marginTop: 8 }}><Button icon={<SaveOutlined />} onClick={() => void savePrompt(prompt)}>保存提示词</Button><Button onClick={() => void regenerate(prompt)} loading={busy}>重新生成</Button><Button type="primary" icon={<PictureOutlined />} disabled={skill === "none"} loading={busy} onClick={() => void generateImage(prompt)}>生成正文图片</Button></Space></Card>)}</Space></Card>
      {assets.length > 0 && <Card title="生成图片" style={{ marginTop: 16 }}><Space wrap>{assets.map((asset) => <div key={asset.id}><Tag>{asset.role === "cover" ? "封面" : "正文"}</Tag><img src={asset.public_url} alt="公众号配图" style={{ width: 160, height: 90, objectFit: "cover", display: "block", marginTop: 6 }} /></div>)}</Space></Card>}
